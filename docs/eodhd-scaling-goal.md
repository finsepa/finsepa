...
Cron schedule: every 15 minutes (`vercel.json`). Market ingest: **hot** keys (quotes/tabs) refresh each live 15m segment; **slow** keys (EOD-derived screener/crypto/indices) use `slow-live-{sessionDay}` once per regular session. Skips when frozen segment is fresh, hot row &lt;14m old, or slow row &lt;20h old — except **`crypto_tab` / `crypto_page2`**, which still refresh every ~15m while US equities are frozen (crypto 24/7; `crypto_derived` stays slow). No `SUPABASE_SERVICE_ROLE_KEY` → full skip (no EODHD burn). Hub ingest skips per-key when segment is fresh (macro/earnings/economy daily/weekly; news ~15m TTL).
