...
const PROFILE_SCREENER_LINK_CLASS =
  "text-[14px] font-normal leading-5 text-fg transition-colors hover:underline";
...
function ProfileCell({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="flex flex-col gap-1">
      <p className="w-full text-[12px] font-semibold leading-5 text-fg-muted">{label}</p>
      <div className="min-h-[1.25rem] text-[14px] font-normal leading-5 text-fg">{children}</div>
    </div>
  );
}
...
function ProfileDuoRow({ left, right }: { left: ReactNode; right: ReactNode }) {
  return (
    <div className="flex flex-col sm:flex-row sm:gap-9">
      <div className="flex-1 border-b border-dashed border-stroke py-2.5">{left}</div>
      <div className="flex-1 border-b border-dashed border-stroke py-2.5">{right}</div>
    </div>
  );
}
...
function LinkValue({ url }: { url: string | null }) {
  ...
  return (
    <a
      ...
      className="break-all text-[14px] font-normal text-fg underline decoration-solid [text-decoration-skip-ink:none] transition-colors hover:text-fg"
    >
      {display}
    </a>
  );
...
function ProfileDescription({ text }: { text: string | null }) {
  ...
  if (!text?.trim()) {
    return <p className="text-[14px] leading-5 text-fg-muted">—</p>;
  }
  ...
  return (
    <div className="flex flex-col gap-2 text-left">
      <div
        ...
        id={contentId}
      >
        <p className="w-full whitespace-pre-wrap text-[14px] font-normal leading-5 text-fg">{text}</p>
      </div>
      {needsMore ? (
        <button
          type="button"
          className="w-full text-left text-[14px] font-normal text-fg underline decoration-solid [text-decoration-skip-ink:none] transition-opacity hover:opacity-80"
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
...
export function StockProfileTab({ ticker, initialProfile }: { ticker: string; initialProfile?: StockProfilePayload | null }) {
  ...
  const themedLogoUrl = useMemo(() => {
    const base = meta.logoUrl?.trim() || "";
    return base ? withLogoDevTheme(base, logoTheme, ticker) : "";
  }, [meta.logoUrl, logoTheme, ticker]);

  useEffect(() => {
    let cancelled = false;
    async function load() {
      // SSR may pass `null` when the parallel fundamentals call failed/timed out.
      // Only skip the client fetch when we already have a real payload.
      ...
      {loading ? (
        <>
          <div className="flex justify-start" aria-hidden>
            <div className="h-[61px] w-full max-w-[250px] rounded-lg bg-skeleton" />
          </div>
          ...
          <div className="w-full space-y-0" aria-hidden>
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="flex flex-col gap-0 sm:flex-row sm:gap-9">
                <div className="flex-1 border-b border-dashed border-stroke py-2.5">
                  <div className="h-3 w-16 rounded bg-skeleton" />
                  <div className="mt-2 h-4 w-3/4 max-w-xs rounded bg-skeleton" />
                </div>
                <div className="flex-1 border-b border-dashed border-stroke py-2.5">
                  <div className="h-3 w-20 rounded bg-skeleton" />
                  <div className="mt-2 h-4 w-2/3 max-w-xs rounded bg-skeleton" />
                </div>
              </div>
            ))}
          </div>
