...
const LINE_TOGGLE_ROWS: {
  ...
  { key: "showMaxLine", label: "Max", ariaLabel: "Show maximum line" },
  { key: "showMinLine", label: "Min", ariaLabel: "Show minimum line" },
  { key: "showBarValues", label: "Values", ariaLabel: "Show values on chart" },
];

/** Key Stats metric modal line chart — max/min guides only (no value labels). */
export const KEY_STATS_METRIC_LINE_TOGGLE_ROWS: {
  key: keyof FundamentalsChartDisplayOptions;
  label: string;
  ariaLabel: string;
}[] = [
...
export function FundamentalsChartSettingsMenu({
  ...
  variant?: "bar" | "line";
  /** Override line-mode toggles (e.g. Key Stats metric modal omits Values). */
  lineToggleRows?: typeof LINE_TOGGLE_ROWS;
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      setMenuAnchor({
        ...
        width: Math.min(window.innerWidth - 32, 240),
      });
    };
    update();
  ...
  const toggleRows =
    variant === "line" ? (lineToggleRows ?? LINE_TOGGLE_ROWS) : BAR_TOGGLE_ROWS;
