...
function SkeletonLine() {
  return (
    <li className="flex items-start gap-2.5">
      <SkeletonBox className="mt-0.5 size-4 shrink-0 rounded-full" />
      <div className="min-w-0 flex-1 space-y-1.5 pt-0.5">
        <SkeletonBox className="h-3.5 max-w-[92%] rounded" />
        <SkeletonBox className="h-3.5 w-[68%] rounded" />
      </div>
    </li>
  );
}
...
export function KeyIndicatorsSkeleton({ className }: { className?: string }) {
  return (
    ...
    >
      <SkeletonBox className="mb-3 h-5 w-28 rounded" />
