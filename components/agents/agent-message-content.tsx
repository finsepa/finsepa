...
const APP_PATH_SPLIT_RE =
  /(\*\*[^*]+\*\*|\/(?:watchlist|portfolio|portfolios|news|screener|earnings|macro|agents|heatmap|heatmaps|economy|crypto|stock)(?:\/[A-Za-z0-9._~%+-]*)*)/g;

const APP_PATH_TEST_RE =
  /^\/(?:watchlist|portfolio|portfolios|news|screener|earnings|macro|agents|heatmap|heatmaps|economy|crypto|stock)(?:\/[A-Za-z0-9._~%+-]*)*$/;
...
function parseAgentBlocks(content: string): Block[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numbered) {
      flushLists();
      flushPara();
      blocks.push({ type: "heading", text: cleanSectionHeading(numbered[1] ?? "") });
      continue;
    }
    ...
    if (
      ...
      if (next && (BULLET_RE.test(next) || isHoldingOrTickerBullet(next))) {
        flushLists();
        flushPara();
      ...
      {blocks.map((b, i) => {
        if (b.type === "heading") {
          return (
            <h3
              key={`h-${i}`}
              className="text-[14px] font-semibold leading-5 text-[#5C5D5F]"
            >
              {b.text}
            </h3>
          );
