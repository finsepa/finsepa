...
function MetricCard({
  ...
  const showCompare = compareEnabled && row.benchmark != null && row.deltaPct != null;
    ...
    >
      ...
      <div className="mt-auto flex flex-wrap items-end justify-between gap-2 pt-3">
        <div className="min-w-0">
          <p className="text-2xl font-semibold leading-8 tracking-tight text-[#09090B]">{row.portfolio}</p>
          {showCompare ? (
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {METRICS.map((row) => (
          <MetricCard key={row.title} row={row} compareEnabled={compareToSpy} />
        ))}
      </div>
    </div>
  );
