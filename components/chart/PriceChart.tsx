...
/** Live 1D: server returns the full session — replace stale SSR/client state on refresh. */
...
export function PriceChart({
  ...
  const [clientMinuteBars, setClientMinuteBars] = useState<StockChartPoint[]>(() => {
    if (holdingsStyle || kind !== "stock" || range !== "1D") return [];
    if (getUsEquityMarketSession(new Date()) !== "regular") return [];
    const sessionYmd = usSessionYmdFromUnixSeconds(Math.floor(Date.now() / 1000));
    return readStock1DSessionBarsFromStorage(symbol, sessionYmd);
  });
  const [liveSessionMinute, setLiveSessionMinute] = useState(() =>
    resolveInitialLiveSessionMinute(liveSessionMinuteProp, initialChart, kind, symbol),
  );
  ...
  useEffect(() => {
    setLiveSessionMinute(resolveInitialLiveSessionMinute(liveSessionMinuteProp, initialChart, kind, symbol));
  ...
  useEffect(() => {
    if (holdingsStyle || kind !== "stock" || range !== "1D") {
      setClientMinuteBars([]);
      return;
    }
    ...
    const sessionYmd = usSessionYmdFromUnixSeconds(Math.floor(Date.now() / 1000));
    setClientMinuteBars(readStock1DSessionBarsFromStorage(symbol, sessionYmd));
  ...
  useEffect(() => {
    ...
    if (!bar) return;
    setClientMinuteBars((prev) => {
      const merged = mergeStockChartPointsByTime([prev, [bar]]);
      writeStock1DSessionBarsToStorage(symbol, sessionYmd, merged);
      return merged;
    });
  }, [holdingsStyle, kind, range, liveSessionMinute, liveSpotUsd, sessionNowMs, symbol]);
  ...
  const dataTimeZoneHint = useMemo(
    () =>
      chartSourcePoints.find((p) => typeof p.timeZone === "string" && p.timeZone.length > 0)?.timeZone ??
      (kind === "stock" ? "America/New_York" : "UTC"),
    [chartSourcePoints, kind],
  );
  ...
  const chartPoints = useMemo(() => {
    ...
    if (prepared.length) {
      return pinLiveSessionChartPointsToSpot(
        prepared,
        liveSpotUsd,
        now,
        liveSessionChartOptions,
    ...
    if (!sessionYmd) return chartSourcePoints;
    ...
    if (!filtered.length) return chartSourcePoints;
  ...
  /** Avoid re-running series setData when only ready flips. */
  ...
  const periodAxisSyncOptionsRef = useRef(periodAxisSyncOptions);
  periodAxisSyncOptionsRef.current = periodAxisSyncOptions;
  ...
  const fitChartTimeScaleRef = useRef(fitChartTimeScale);
  fitChartTimeScaleRef.current = fitChartTimeScale;
  ...
  const crosshairForHeader = useMemo(():
    | { price: number; timeUnix: number; periodLabel?: string | null }
    | null => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setHoverTimeUnixGuarded(null);
      setReady(false); readyRef.current = false;
      // Clear immediately so we never setData/fit prior-range dense bars (BTC 24H) under the new range.
      setPoints([]);
        ...
        if (nextPoints.length) {
          setPoints((prev) =>
            applyStockChartApiPoints(
              prev,
              nextPoints,
              kind === "stock" && range === "1D" && !holdingsStyle && liveSessionMinuteRef.current,
            ),
          );
        } else {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (next.length && !cancelled) {
          setPoints((prev) =>
            applyStockChartApiPoints(prev, next, liveSessionMinuteRef.current),
          );
        }
      } catch {
        /* ignore */
  ...
  useLayoutEffect(() => {
    ...
    const useLiveSessionChart = stock1DLiveSessionRef.current;
    ...
    const open =
      useLiveSessionChart && liveSessionYmd
        ? (resolveSessionOpenAnchorValue(
            ...
            liveSessionYmd,
            timeZone,
            liveSessionMinuteRef.current ? null : liveSpotUsdRef.current,
            new Date(),
            { liveSessionMinute: liveSessionMinuteRef.current },
    ...
    if (useLiveSessionChart && liveSessionYmd) {
      ...
      fitChartTimeScaleRef.current(chart, plotW, data.length);
    }
          ...
          if (!holdingsStyle && !loadingRef.current && chartRef.current && hoverTimeRef.current == null) {
            setPeriodAxisLabelsGuarded(
              syncOverviewPeriodAxisLabels(
                ...
                overviewBottomAxisModeRef.current,
                containerWidthRef.current,
                periodAxisSyncOptionsRef.current,
              ),
            );
          }
        });
  ...
  }, [chartPoints, lastPointStroke, holdingsStyle, tradeMarkers, holdingsQuarterBands, costBasisPrice, kind, series, symbol, screenshotPreviewMode, chartThemePaintKey]);
