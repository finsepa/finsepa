"use client";

import type { ReactNode } from "react";

import { cn } from "@/lib/utils";

import { AuthBrandMark } from "./auth-brand-mark";
import { AuthCardHeaderProvider } from "./auth-card-header";
import { AuthCornerActions } from "./auth-corner-actions";
import { AuthPreCardBannerProvider } from "./auth-pre-card-banner";
import { AuthSplitAsidePanelLazy } from "./auth-split-aside-lazy";
import { AuthSplitLayout } from "./auth-split-layout";

function AuthHeaderBlock({
  title,
  subtitle,
  titleClassName,
  leading,
  brand,
}: {
  title: string;
  subtitle: ReactNode;
  titleClassName?: string;
  leading?: ReactNode;
  /** `undefined` = default logo; `null` = hidden; otherwise custom mark. */
  brand?: ReactNode | null;
}) {
  const brandNode = brand === undefined ? <AuthBrandMark /> : brand;
  return (
    <div className="relative flex flex-col items-center gap-4">
      {leading ? (
        <div className="absolute left-0 top-0 z-10">{leading}</div>
      ) : null}
      {brandNode}
      <div className="flex w-full flex-col gap-2 text-center">
        <h1
          className={cn(
            "text-[26px] font-semibold leading-8 tracking-tight text-fg",
            titleClassName,
          )}
        >
          {title}
        </h1>
        {subtitle ? <div className="text-sm leading-5 text-fg-muted">{subtitle}</div> : null}
      </div>
    </div>
  );
}

export function AuthCenteredLayout({
  title,
  subtitle,
  titleClassName,
  children,
  preCard,
  compact = false,
  belowCard,
  footer,
  split = false,
  cornerActions = false,
}: {
  title: string;
  subtitle: ReactNode;
  titleClassName?: string;
  children: ReactNode;
  preCard?: ReactNode;
  compact?: boolean;
  belowCard?: ReactNode;
  footer?: ReactNode;
  split?: boolean;
  /** Theme + help controls (login / signup / forgot password). */
  cornerActions?: boolean;
}) {
  const formWidthClass = split ? "max-w-[384px]" : compact ? "max-w-[360px]" : "max-w-[420px]";

  return (
    <AuthPreCardBannerProvider initial={preCard ?? null}>
      {(banner) => (
        <AuthCardHeaderProvider>
          {(headerOverride) => {
            const headerTitle = headerOverride?.title ?? title;
            const headerSubtitle =
              headerOverride != null ? (headerOverride.subtitle ?? null) : subtitle;
            const headerLeading = headerOverride?.leading;
            const headerBrand =
              headerOverride != null && "brand" in headerOverride
                ? headerOverride.brand
                : undefined;

            const formBlock = (
              <div className={cn("flex w-full flex-col gap-4", formWidthClass)}>
                {banner}
                <AuthHeaderBlock
                  title={headerTitle}
                  subtitle={headerSubtitle}
                  titleClassName={titleClassName}
                  leading={headerLeading}
                  brand={headerBrand}
                />
                <div className={compact ? "space-y-4" : "space-y-6"}>{children}</div>
              </div>
            );

            if (split) {
              return (
                <>
                  <AuthSplitLayout
                    form={formBlock}
                    aside={<AuthSplitAsidePanelLazy />}
                    footer={footer ?? undefined}
                  />
                  {cornerActions ? <AuthCornerActions /> : null}
                </>
              );
            }

            return (
              <>
                <div className="fixed inset-0 -z-10 bg-nav md:hidden" aria-hidden />
                <main className="flex min-h-[100dvh] flex-col bg-nav">
                  <div className="flex flex-1 flex-col items-center justify-center p-4">
                    <div className={cn("flex w-full flex-col", formWidthClass)}>
                      <div className="flex flex-col gap-3">
                        {banner}
                        <div
                          className={cn(
                            "rounded-[12px] border border-stroke-subtle bg-surface shadow-[0px_1px_2px_0px_rgba(var(--fs-shadow-rgb),var(--fs-shadow-a-06))]",
                            compact ? "p-6" : "p-8",
                          )}
                        >
                          <div className={compact ? "mb-4" : "mb-6"}>
                            <AuthHeaderBlock
                              title={headerTitle}
                              subtitle={headerSubtitle}
                              titleClassName={titleClassName}
                              leading={headerLeading}
                              brand={headerBrand}
                            />
                          </div>
                          <div className={compact ? "mt-4" : "mt-6"}>{children}</div>
                        </div>
                      </div>
                      {belowCard != null ? <div className="mt-4">{belowCard}</div> : null}
                    </div>
                  </div>
                  {footer != null ? (
                    <div className="shrink-0 px-4 pb-8 pt-2">
                      <div className={cn("mx-auto w-full text-center", formWidthClass)}>{footer}</div>
                    </div>
                  ) : null}
                </main>
                {cornerActions ? <AuthCornerActions /> : null}
              </>
            );
          }}
        </AuthCardHeaderProvider>
      )}
    </AuthPreCardBannerProvider>
  );
}
