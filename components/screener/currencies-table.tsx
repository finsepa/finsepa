...
import { currencyAssetHref } from "@/lib/market/currency-page-shared";
import type { CurrencyTableRow } from "@/lib/screener/screener-currencies-universe";
import { SCREENER_CURRENCIES_PAGE_SIZE } from "@/lib/screener/screener-markets-page-size";
import { forexWatchlistKey } from "@/lib/watchlist/constants";
import { useWatchlist } from "@/lib/watchlist/use-watchlist-client";
import { cn } from "@/lib/utils";

function formatFxRate(v: number): string {
  if (!Number.isFinite(v)) return "-";
  const digits = v >= 20 ? 2 : 4;
...
export function CurrenciesTable({
  ...
  if (safeRows.length === 0) {
    return <IndicesTableSkeleton rows={SCREENER_CURRENCIES_PAGE_SIZE} />;
  }
          ...
          <div className={SCREENER_TABLE_ROW_HOVER_PAD_CLASS}>
            <div className="flex min-h-[44px] min-w-0 w-full items-center gap-x-1.5 py-0 text-[14px] font-medium leading-5 text-fg-muted sm:gap-x-2">
              ...
              <div className={cn(rowGrid, "min-h-[44px] w-full items-center")}>
                ...
                <div className="min-w-0 w-full text-left">Currency Pair</div>
