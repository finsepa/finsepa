...
const TopbarPortfolioBlock = memo(function TopbarPortfolioBlock() {
  ...
  return (
    <TopbarDelayedTooltip label="My Portfolio" className="max-w-full min-w-0 shrink-0">
      <div className="flex h-8 max-w-full min-w-0 items-stretch overflow-visible rounded-[10px] border border-[#E4E4E7] bg-white shadow-[0px_1px_2px_0px_rgba(10,10,10,0.06)] sm:h-9">
        <Link
          href="/portfolio"
    ...
    <>
      <header className="flex min-h-[52px] min-w-0 items-center justify-between gap-1.5 px-2 py-2 sm:min-h-[60px] sm:gap-3 sm:px-4 sm:py-3">
        <div className="flex min-w-0 flex-1 items-center sm:gap-3">
          <TopbarDelayedTooltip label="Search" className="min-w-0 flex-1 sm:max-w-[300px]">
            <button
              type="button"
              onClick={() => setSearchOpen(true)}
              aria-label="Search (shortcut S)"
              className="flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-lg bg-[#F4F4F5] transition-all duration-100 hover:bg-[#EBEBEB] sm:h-9 sm:min-w-0 sm:w-full sm:justify-start sm:gap-2 sm:px-4 sm:text-left"
            >
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-3">
          ...
          <TopbarDelayedTooltip label="Watchlist">
            ...
            >
              <Star className="h-4 w-4 sm:h-5 sm:w-5" aria-hidden />
            </Link>
          </TopbarDelayedTooltip>
          ...
          <TopbarDelayedTooltip label="Help">
            <a
              href="mailto:hi@finsepa.com"
              className={`${topbarSquircleIconClass} hidden sm:inline-flex`}
