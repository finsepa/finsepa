...
export const topbarSquircleIconClass = `flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] ${whiteSurfaceButtonChromeClass} text-[#09090B] transition-all duration-100 hover:bg-[#F4F4F5]`;

/** Label + icon, same surface as portfolio strip / squircle row (variable width). */
export const topbarSquircleTextButtonClass = `inline-flex h-9 shrink-0 items-center gap-2 rounded-[10px] ${whiteSurfaceButtonChromeClass} px-3 text-sm font-medium leading-5 text-[#09090B] transition-all duration-100 hover:bg-[#F4F4F5]`;
