...
import {
  ...
  reorderWatchlistCollectionItemsOnServer,
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const rawTickers = (body as { tickers: unknown }).tickers;
    if (!Array.isArray(rawTickers)) {
      return NextResponse.json({ error: "tickers must be an array." }, { status: 400 });
    }

    const tickers = rawTickers.filter((ticker): ticker is string => typeof ticker === "string");
    await reorderWatchlistCollectionItemsOnServer(supabase, user.id, id, tickers);
