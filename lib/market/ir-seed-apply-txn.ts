...
import { isTxnIrUrl, mergeTxnKnownQuarterDocs } from "@/lib/market/ir-seed-txn-match";
...
/**
 * TXN: no quarterly deck — Slides stay empty. Earnings-release static-files as Filings.
 * Never 10-Q / 8-K wrapper.
 */
export async function applyIrSeedTxnDocumentUrls(
  rows: StockEarningsHistoryRow[],
  _hub: StockEarningsDocumentHub,
): Promise<StockEarningsHistoryRow[]> {
  const byLabel = mergeTxnKnownQuarterDocs();
  return rows.map((row) => {
    ...
    const nextFilings =
      hit?.filings && isDirectEarningsPdfUrl(hit.filings) && isTxnIrUrl(hit.filings) ? hit.filings : null;
