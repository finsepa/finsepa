...
import { isCIrPdf, mergeCKnownQuarterDocs } from "@/lib/market/ir-seed-c-match";
...
/**
 * C: preso/psqtr as Slides; prqtr press as Filings.
 * Never at-a-glance infographic, 8-K wrapper, or transcript.
 */
export async function applyIrSeedCDocumentUrls(
  rows: StockEarningsHistoryRow[],
  _hub: StockEarningsDocumentHub,
): Promise<StockEarningsHistoryRow[]> {
  const byLabel = mergeCKnownQuarterDocs();
  return rows.map((row) => {
    ...
    const nextSlides =
      hit?.slides && isDirectEarningsPdfUrl(hit.slides) && isCIrPdf(hit.slides) ? hit.slides : null;
    const nextFilings =
      hit?.filings && isDirectEarningsPdfUrl(hit.filings) && isCIrPdf(hit.filings) ? hit.filings : null;
