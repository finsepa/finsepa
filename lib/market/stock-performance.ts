...
/** Compute performance table from pre-fetched daily bars (ascending recommended). */
export function computeStockPerformanceFromSortedDailyBars(
  ...
  const prev1dClose = closeAtTradingOffset(sorted, 1);
  const prev5dClose = closeAtTradingOffset(sorted, 5);
  const prev7dClose = closeAtTradingOffset(sorted, 7);
