...
export async function loadStockNewsUncached(ticker: string): Promise<StockNewsArticle[]> {
  ...
  if (process.env.DEBUG_STOCK_NEWS === "1") {
    for (let d = 0; d < Math.min(3, rows.length); d++) {
      const r = rows[d]!;
      const keys = Object.keys(r).sort();
      const imageKeys = keys.filter((k) => /image|thumb|photo|banner|urlToImage|media|picture/i.test(k));
      console.info(
        `[stock-news] item ${d} provider keys: ${keys.join(", ")} | image-related keys: ${imageKeys.length ? imageKeys.join(", ") : "(none)"}`,
      );
    }
  }
