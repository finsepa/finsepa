...
import {
  CAT_IR_PAGES,
  mergeCatKnownQuarterDocs,
  parseCatQuarterlyResultsHtml,
} from "@/lib/market/ir-seed-cat-match";
...
/**
 * CAT: q4cdn analyst slide deck as slides, earnings-release PDF as filings.
 * Never lock SEC HTML, transcripts, or caterpillar.com news HTML.
 */
export async function applyIrSeedCatDocumentUrls(
  ...
  for (const page of CAT_IR_PAGES) {
    const html = await fetchHtml(page);
    if (!html) continue;
    for (const [label, docs] of parseCatQuarterlyResultsHtml(html, page)) {
  ...
  const byLabel = mergeCatKnownQuarterDocs(parsed);
