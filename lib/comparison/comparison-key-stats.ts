...
export function comparisonGrowthCellClassName(
  ...
  if (!isComparisonGrowthMetricId(metricId)) {
    return `${baseClass} font-normal text-fg`;
  }
  const tone = comparisonGrowthCellTone(cell);
  if (tone === "positive") return `${baseClass} font-normal text-up`;
  if (tone === "negative") return `${baseClass} font-normal text-down`;
  if (tone === "missing") return `${baseClass} font-normal text-fg-muted`;
  return `${baseClass} font-normal text-fg`;
}
