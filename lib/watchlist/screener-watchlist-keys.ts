...
import { SCREENER_CURRENCY_MAJORS } from "@/lib/screener/screener-currencies-universe";
...
/** Sync slice: page-1 stocks + crypto + indices + currencies (used where async isn’t available). */
export function buildScreenerWatchlistKeySetSync(): Set<string> {
  ...
  for (const { symbol } of SCREENER_CURRENCY_MAJORS) s.add(forexWatchlistKey(symbol));
  return s;
}
