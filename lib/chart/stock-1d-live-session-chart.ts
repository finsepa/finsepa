...
export type Stock1DLiveSessionChartOptions = {
  /** False on US holidays — plot the last completed session instead of today's clock window. */
  liveSessionMinute?: boolean;
};

function stock1DUsesLiveSessionClock(now: Date, options?: Stock1DLiveSessionChartOptions): boolean {
  return options?.liveSessionMinute !== false && getUsEquityMarketSession(now) === "regular";
}

export { stock1DUsesLiveSessionClock };

function stock1DSessionEndSec(
  sessionYmd: string,
  timeZone: string,
  now: Date,
  ...
  if (!stock1DUsesLiveSessionClock(now, options)) return close;
  const nowSec = Math.floor(now.getTime() / 1000);
  return Math.min(nowSec, close);
}
...
/** 5m buckets for sparse intraday (BAC, RACE, etc.) — smooth like pre–1m rollout charts. */
export const STOCK_1D_LIVE_SESSION_SPARSE_BAR_INTERVAL_SEC = 300;

/** 15m buckets when very few source points exist. */
export const STOCK_1D_LIVE_SESSION_VERY_SPARSE_BAR_INTERVAL_SEC = 900;
...
/** During live regular session, pin to today's US session; otherwise use the payload session. */
...
function dropIsolatedLiveTailAnchors(
  ...
  if (
    ...
    last.time - prev.time > STOCK_1D_LIVE_SESSION_SPARSE_BAR_INTERVAL_SEC
  ) {
...
function sessionSourceStats(
  ...
  open: number,
  endSec: number,
): { coverage: number; maxGapSec: number; pointCount: number } {
  if (!sorted.length) {
    return { coverage: 0, maxGapSec: Number.POSITIVE_INFINITY, pointCount: 0 };
  }
  let maxGap = Math.max(0, sorted[0]!.time - open);
  for (let i = 1; i < sorted.length; i++) {
    maxGap = Math.max(maxGap, sorted[i]!.time - sorted[i - 1]!.time);
  }
  const expected = Math.max(
    1,
    Math.floor((endSec - open) / STOCK_1D_LIVE_SESSION_BAR_INTERVAL_SEC) + 1,
  );
  return {
    coverage: sorted.length / expected,
    maxGapSec: maxGap,
    pointCount: sorted.length,
  };
}

function typicalSourceGapSec(sorted: readonly StockChartPoint[]): number {
...
export function isUltraSparseLiveSessionSource(
  ...
  if (inSession.length <= 3) {
    const span = inSession[inSession.length - 1]!.time - inSession[0]!.time;
    return span >= 20 * 60;
  }
  return false;
}
...
export function resolveSessionOpenAnchorValue(
  points: readonly StockChartPoint[],
  sessionYmd: string,
  timeZone: string,
  ...
  if (lastBefore != null) {
    ...
    if (!stock1DUsesLiveSessionClock(now, options) || sameSessionDay) {
...
/** Pin the visible line end to the current wall-clock time (never beyond `now`) during live session. */
...
/** Client live-price poll during regular session — see {@link STOCK_1D_LIVE_PRICE_POLL_MS} in stock-live-poll-intervals. */
export {
  STOCK_1D_LIVE_CHART_POLL_MS,
  STOCK_1D_LIVE_PRICE_POLL_MS,
} from "@/lib/chart/stock-live-poll-intervals";
...
export function prepareStock1DLiveSessionChartPoints(
  ...
  const timeZone = STOCK_1D_LIVE_SESSION_TZ;
  const session = getUsEquityMarketSession(now);
  const useLiveSpot = stock1DUsesLiveSessionClock(now, options);
  const sessionYmd = resolveStock1DLiveSessionYmd(points, timeZone, now, options);
  if (!sessionYmd) return [...points];

  const sourcePoints = collectStock1DLiveSessionSourcePoints(points, sessionYmd, timeZone, now, options);
  ...
  if (useLiveSpot) {
    regularPoints = appendLiveSessionNowTail(sessionPoints, tailValue, sessionYmd, timeZone, now, options);
  } else {
    const close = usSessionWallClockUnix(sessionYmd, 16, 0, timeZone);
...
export function fitOverviewChartTimeScale(
  ...
  if (shouldUseStock1DLiveSessionChart(ctx.kind, ctx.range, ctx.points, ctx.holdingsStyle)) {
    const ymd = resolveStock1DLiveSessionYmd(ctx.points, ctx.timeZone, new Date(), {
      liveSessionMinute: ctx.liveSessionMinute,
    });
    if (ymd) {
